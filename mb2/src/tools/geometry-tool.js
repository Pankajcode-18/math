...
const GeometryTool = (() => {
  ...
  function drawSnapIndicator(ctx, snap) {
    ...
    // Crosshair
    ctx.beginPath();
    ctx.moveTo(snap.x - 12, snap.y);
    ctx.lineTo(snap.x + 12, snap.y);
    ctx.moveTo(snap.x, snap.y - 12);
    ctx.lineTo(snap.x, snap.y + 12);
    ctx.stroke();

    if (snap.name) {
      ctx.font = 'bold 10px monospace';
      ctx.fillStyle = '#38bdf8';
      ctx.fillText(snap.name, snap.x + 10, snap.y - 10);
    }
    ctx.restore();
  }
  ...
  function drawBadge(ctx, x, y, text, color = '#38bdf8') {
    ...
    ctx.lineWidth = 1.2;
    ctx.beginPath();
    if (ctx.roundRect) ctx.roundRect(x - pw/2, y - ph/2, pw, ph, 6);
  ...
  function renderAnglePreview() {
    ...
    if (state.step === 1) {
      ...
      // Endpoint A dot
      ctx.fillStyle = '#38bdf8';
      ctx.beginPath(); ctx.arc(pA.x, pA.y, 5, 0, Math.PI * 2); ctx.fill();
    ...
    if (state.step === 2) {
      ...
      ctx.strokeStyle = color;
      ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(pA.x, pA.y, 4.5, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      ctx.font = 'bold 12px Inter, sans-serif';
      ctx.fillStyle = color;
      ctx.fillText('A', pA.x + 8, pA.y - 8);
      ...
      if (distB > 2) {
        ...
        ctx.beginPath(); ctx.arc(pB.x, pB.y, 5, 0, Math.PI * 2); ctx.fill();
        ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 2; ctx.stroke();
        ctx.font = 'bold 12px Inter, sans-serif';
        ctx.fillStyle = '#f59e0b';
